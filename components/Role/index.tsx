...
export default function Role({ curPage = 1 }: RoleProps) {
	...
	const tableData = !isLoading
		? !isLoading && !isPageExist
			? []
			: data?.users
		: [];
	const T_data = tableData.map((item: AdminsType) => {
		return {
			member_name: item.firstName
				? item.firstName
				: '' + ' ' + item.lastName
				? item.lastName
				: '',
			...
			roles2:
				item.role === ROLE.ADMINISTRATOR || item.role === ROLE.SUPER_ADMIN
					? 'Owner'
					: '',
	...
	return (
		<div className='w-auto m-0 p-0'>
			...
			>
				<div
					className='w-[80%] absolute top-[5%] mx-auto left-[10%]
				...
				>
					...
					<div className='mt-3 bg-[#1C1C1C] w-full flex md:justify-between max-md:flex-col relative'>
						<div className='w-[50%] max-md:w-full px-2 py-3'>
							<p className='text-[#BCBBB9] text-base mt-3'>
								Select team member roles
							</p>
							<div
								className='bg-[#161616] px-4 py-2 flex flex-row items-center rounded-lg
                                 border-[#717171] border-solid border-[1px] mt-2'
							>
								<Image src={searchImg} width={16} alt={'search'} />
								<input
									type='text'
									className='px-4 text-[#717171] text-sm bg-[#161616] rounded-sm
                                focus:outline-none font-medium placeholder:text-[#717171] placeholder:text-sm max-sm:w-full'
									placeholder='example@gmail.com'
									value={email}
									onChange={(e) => setEmail(e.target.value)}
								/>
							</div>
							<h1 className='my-3 text-[#BCBBB9] text-lg font-medium'>
								Administrator Role
							</h1>
							<div className='mt-3'>
								<CheckComponent
							...
							<div className='mt-2'>
								<CheckComponent
									section={'Help desk member'}
									curRole={curRole}
									setCurRole={setCurRole}
									myRole={ROLE.HELP_DESK_MEMBER}
								/>
							</div>
							<h1 className='my-3 text-[#BCBBB9] text-lg font-medium'>
								Developer Roles
							...
							<div className='mt-2'>
								<CheckComponent
									section={'Litigation Analyst'}
									curRole={curRole}
									setCurRole={setCurRole}
									myRole={ROLE.LITIGATION_ANALYST}
								/>
							</div>
							<div className='mt-2'>
								<CheckComponent
									section={'Refunds Analyst'}
									curRole={curRole}
									setCurRole={setCurRole}
									myRole={ROLE.REFUNDS_ANALYST}
								/>
							</div>
							<h1 className='my-3 text-[#BCBBB9] text-lg font-medium'>
								Read Only Roles
						...
						<div className='w-[50%] max-md:w-full max-md:mt-4 px-2 py-3'>
							...
							<textarea
								className='text-sm placeholder:text-[#717171] mt-4 rounded-md p-4 text-[#717171] focus:outline-none
							...
							<div className='md:bottom-12 flex items-center max-[318px]:justify-center  min-[318px]:flex-row max-[318px]:flex-col  max-md:w-full md:w-[50%] md:absolute'>
								<div className='min-[318px]:ml-auto'>
									...
									>
										<p
											className='text-black font-bold text-sm max-sm:text-xs 
                                            '
										>
											Cancel
										</p>
			...
			<div className=' flex flex-row w-full pt-[70px]'>
				...
				<div className=' w-full bg-[#141414] pb-10 pl-[300px] max-lg:pl-0'>
					<div className='pt-5 px-5'>
						<h1 className='text-white text-lg font-bold leading-3'>Roles</h1>
					...
					<div className='px-5 pt-8 flex flex-row items-center'>
						<div className='max-sm:w-[55%]'>
							<div
								className='ml-5 max-sm:ml-0 px-4 py-2 bg-[#161616] flex flex-row items-center rounded-lg
                                 border-[#717171] border-solid border-[1px]'
							>
								<Image src={searchImg} width={16} alt={'search'} />
								<input
									type='text'
									className='pl-2 text-[#717171] text-sm bg-[#161616] rounded-sm
                                focus:outline-none font-medium placeholder:text-[#717171] placeholder:text-sm max-sm:w-full'
									placeholder='Search'
									value={search}
							...
							<div className='flex flex-row items-center'>
								<div className='sm:hidden'>
									...
									>
										...
										<p
											className='ml-2 text-[#717171] text-sm 
                                            max-sm:hidden'
										>
											Import
										</p>
					...
					{isLoading ? (
						<div className='pt-5 mt-4 flex w-full justify-center  align-middle	'>
							...
							<div className='px-5 pt-8 grid grid-cols-4 gap-0 max-md:grid-cols-3 max-sm:grid-cols-2 pb-6'>
								<div className=''>
									...
									{T_data.map((m: RoleTableDataProps, index: number) => {
										return (
											<div
												key={index}
												className='mt-2 flex flex-col items-center rounded-l-lg bg-[#1B1B1B] h-[70px] pl-2 justify-center'
											>
												<p className='text-white text-base max-sm:text-sm'>
													{m?.member_name}
												</p>
												<p className='text-[#717171] text-sm max-sm:text-xs font-bold'>
													{m?.member_email}
												</p>
											</div>
										);
								...
								<div className='max-md:hidden'>
									...
									{T_data.map((m: RoleTableDataProps, index: number) => (
										<div
											key={index}
											className='mt-2 flex flex-col items-center bg-[#1B1B1B] h-[70px] pl-2 justify-center'
										>
											<p className='text-white text-lg'>{m?.roles1}</p>
											<p className='text-[#717171] text-sm font-bold'>
												{m?.roles2}
											</p>
										</div>
									))}
								...
								<div className=''>
									...
									{T_data.map((m: RoleTableDataProps, index: number) => (
										...
										>
											<button
												className='text-[#55BA68] text-sm rounded-full px-3 py-1 bg-[#202B22]
                                        '
											>
												{m?.authorization}
											</button>
								...
								<div className='max-sm:hidden'>
									...
									{T_data.map((m: RoleTableDataProps, index: number) => (
										<div
											key={index}
											className='mt-2 flex flex-row items-center bg-[#1B1B1B] h-[70px] pl-2'
										>
											<p className='text-[#717171] text-sm font-bold'>
												{m?.lastconnection}
											</p>
