...
const StatisticsLineChat = dynamic(() => import('./statisticsLineChart'), {
	ssr: false,
});
const StatisticsMapChart = dynamic(() => import('./statisticsMapChart'), {
	ssr: false,
});
const StatisticsDoghnut = dynamic(() => import('./statisticsDoughnut'), {
	ssr: false,
});
const StatisticsColumnChart = dynamic(() => import('./statisticsColumn'), {
	ssr: false,
});
...
export default function TransferIndex() {
	...
	const {
		...
		error: saleError,
	} = useSalesStatistic({ type: STATISTIC.FLASH, date_type: DURING.YEAR });
