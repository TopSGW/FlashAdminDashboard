		...
		<div className='fixed w-[300px] bg-[#1B1B1B] max-lg:hidden h-full'>
			<div className='relative w-full h-full'>
				<div className='flex flex-col'>
					<SidebarItem
						Imgurl={orders[0] ? materialSvg : materialsymbol}
						section={'OverView'}
						color={orders[0] ? '#FBBF04' : '#1B1B1B'}
					/>
					<SidebarItem
						Imgurl={orders[1] ? statisticsSvg : statics}
						section={'Statistics'}
						color={orders[1] ? '#FBBF04' : '#1B1B1B'}
					/>
					<SidebarItem
						Imgurl={orders[3] ? buillingInvoiceSvg : billingInvoice}
						section={'Billing & Invoices'}
						color={orders[3] ? '#FBBF04' : '#1B1B1B'}
					/>
					<SidebarItem
						Imgurl={orders[9] ? newclientB : newclientW}
						section={'Client History'}
						color={orders[9] ? '#FBBF04' : '#1B1B1B'}
					/>
					<SidebarItem
						Imgurl={orders[7] ? newclientB : newclientW}
						section={'New Account'}
						color={orders[7] ? '#FBBF04' : '#1B1B1B'}
					/>
					<SidebarItem
						Imgurl={orders[4] ? allTransactionSvg : allTransaction}
						section={'All Transactions'}
						color={orders[4] ? '#FBBF04' : '#1B1B1B'}
					/>
					...
					<SidebarItem
						Imgurl={orders[6] ? rolesidebarImg : roleSvg}
						section={'Role'}
						color={orders[6] ? '#FBBF04' : '#1B1B1B'}
					/>
					<SidebarItem
						Imgurl={notification}
						section={'Notification'}
						color={orders[8] ? '#FBBF04' : '#1B1B1B'}
					/>
				...
				<div className=' absolute bottom-[100px] w-full'>
					<button
                        ...
                        w-[180px] mx-auto rounded-md bg-[#252525]'
						onClick={() => logout.mutate()}
					>
						<Image src={loginImg} width={14} alt={''} />
						<p className='text-[#BCBBB9] text-sm ml-2'>Log Out</p>
					</button>
				</div>
			</div>
		</div>
